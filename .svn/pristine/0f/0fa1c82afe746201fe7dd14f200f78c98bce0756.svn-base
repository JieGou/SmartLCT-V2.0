...
namespace SmartLCT
{
    class MainWindow_VM : SmartLCTViewModeBase
    {
        ...
        public ImageState IncreaseOrDecreaseState
        {
            get
            {
                return _increaseOrDecreaseState;
            }
            set
            {
                _increaseOrDecreaseState = value;
                NotifyPropertyChanged(GetPropertyName(o => this.IncreaseOrDecreaseState));
            }
        }
        private ImageState _increaseOrDecreaseState = ImageState.Normal;
        ...
        private void IniCmd()
        {
            ...
            CmdHelp = new RelayCommand(OnCmdHelp);
        }
            ...
            else if (SelectedEnvironMentIndex == 1)
            {
                ...
                else if (senderCount == 1)
                {
                   ...
                   RectLayer screenBaseRectLayer = new RectLayer(0, 0, screenLayerSize.Width, screenLayerSize.Height, screenLocationRectLayer, 0, ElementType.screen, 0);
                    ...
                    IniDisplayScreen(senderIndex, screenLocationRectLayer, screenBaseRectLayer);
                   // //由每个屏的发送卡带载生成各个显示屏带载
                   ///////
                   // for (int i = 0; i < MyScreen.ElementCollection.Count; i++)
                   // {
        ...
        private void OnCreateScreenWithConfigurationData(ConfigurationData data)
        {
            ...
            RectLayer screen;
            if (SelectedValue.ElementCollection.Count > 0)
            {
                screen = (RectLayer)SelectedValue.ElementCollection[0];
            }
            else
            {
                return;
        ...
        private void OnIncreaseOrDecreaseIndexChanged(int index)
        {
            ...
            GetImageState();
            if (SelectedEnvironMentIndex == 1 && _increaseOrDecreaseState==ImageState.Normal)
            {
                int increaseOrDecreaseIndex=((IRectLayer)ScreenLocationRectLayer.ElementCollection[0]).IncreaseOrDecreaseIndex;
                MyScreen.SenderConnectInfoList[ScreenLocationRectLayer.SenderIndex].IncreaseOrDecreaseIndex = increaseOrDecreaseIndex;
                Point newLocation = GetDviCenterPoint(increaseOrDecreaseIndex, (IRectLayer)ScreenLocationRectLayer.ElementCollection[0]);
        ...
        private void OnCmdChangedLang(string langFlag)
        {
            ...
            NotifyScannerConfigInfoChanged();
        ...
        #region Help
        private string GetChmFileName(string filePath,string strExten)
        {
            ...
            if (Directory.Exists(filePath))
            {
                ...
                if (directoryInfo.GetFiles().Count() > 0)
                {
                    var files = (from file in directoryInfo.GetFiles()
                                 where file.Extension == strExten
                                 select file.Name).FirstOrDefault();
                    if (files != null)
                    {
        ...
        private string GetChmPath()
        {
            string chmPath =Path.Combine(AppDomain.CurrentDomain.BaseDirectory.ToString() ,"Resource\\Help");
            string lang = _globalParams.AppConfig.LangFlag;
            chmPath = Path.Combine(chmPath, lang);
            chmPath = Path.Combine(chmPath, GetChmFileName(chmPath, ".chm"));
            if (!File.Exists(chmPath))
            {
                chmPath = string.Empty;
            }
            return chmPath;
        }
        private void OnCmdHelp()
        {
            string chmPath = GetChmPath();
            if (chmPath != string.Empty)
            {
