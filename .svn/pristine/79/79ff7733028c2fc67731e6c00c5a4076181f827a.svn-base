...
namespace Nova.SmartLCT.UI.ViewModel
{
    public class SplashWindowViewModel : SmartLCTViewModeBase
    {
        #region 自定义结构
        /// <summary>
        /// 当前的系统类型
        ...
        public string LoadingMessage
        {
            ...
            {
                _loadingMsg = value;
                NotifyPropertyChanged(GetPropertyName(o => this.LoadingMessage));
            }
        }
        ...
        #region 命令
        public RelayCommand InitializeEnvironmentCommand
        {
            get;
            private set;
        }
        ...
        public SplashWindowViewModel()
        {
            if (!this.IsInDesignMode)
            {
                ...
                InitializeEnvironmentCommand = new RelayCommand(InitializeEnvironment);
            ...
            DispatcherHelper.UIDispatcher.BeginInvoke(new Action(() =>
                {
                    ...
                    StartM3ServiceProcess();
                    CustomTransform.Delay(200, 10);

                    StartBrightAdjustToolProcess(ConstValue.BRIGHT_ADJUST_TOOL_PROC_NAME, ConstValue.BRIGHT_ADJUST_TOOL_START_FILE);
                    CustomTransform.Delay(200, 10);

                    LoadUserConfig();
                    CustomTransform.Delay(200, 10);

                    InitalizeM3ServiceProxy();

                    Messenger.Default.Send<string>(string.Empty, MsgToken.MSG_STARTUP_COMPLETED);
                }));
        }
        ...
        /// </summary>
        /// <returns></returns>
        private bool InitalizeM3ServiceProxy()
        {
            LoadingMessage = GetLocalizationMessage("正在注册服务...", "Lang_StartUp_RegistServer");
        ...
        {
            LoadingMessage = GetLocalizationMessage("正在获取数据...", "Lang_StartUp_GetData");
            ...
            sqlLite.GetAllScannerCofig().ForEach(o => _globalParams.ScannerConfigCollection.Add(o));
        }

        /// <summary>
        /// 重新启动M3服务
        /// </summary>
        private void StartM3ServiceProcess()
        {
            LoadingMessage = GetLocalizationMessage("正在启动服务...", "Lang_StartUp_StartServer");
        ...
        private static string GetLocalizationMessage(string defaultMessage, string messageKey)
        {
            string msg = string.Empty;
            CommonStaticMethod.GetLanguageString(defaultMessage, messageKey, out msg);
            return msg;
        }
        ...
        private void StartBrightAdjustToolProcess(string processName, string filePath)
        {
            LoadingMessage = GetLocalizationMessage("正在启动亮度调节软件...", "Lang_StartUp_StartBright");
            try
            {
